...
function validateFile(file: File): string | null {
  ...
  if (file.size > MAX_FILE_BYTES) return "The selected file exceeds the 25 MB limit.";
  return null;
}
      ...
      {result && (
        <section className="results-panel" aria-live="polite">
          ...
          <p className="evidence-note">
            These are diagnostic proxies, not a real/fake verdict. Classification begins in Week 2.
          </p>
          <div className="metrics-grid">
            <div><span>RT60 proxy</span><strong>{result.rt60_estimate_sec < 0 ? "N/A" : `${result.rt60_estimate_sec.toFixed(3)} s`}</strong></div>
            <div><span>Reverb ratio</span><strong>{result.reverb_ratio.toFixed(4)}</strong></div>
            <div><span>Breathing-band ratio</span><strong>{result.breathing_band_energy.toFixed(4)}</strong></div>
            <div><span>Duration</span><strong>{result.waveform_summary.duration_sec.toFixed(2)} s</strong></div>
            <div><span>Peak amplitude</span><strong>{result.waveform_summary.peak_amplitude.toFixed(4)}</strong></div>
            <div><span>RMS</span><strong>{result.waveform_summary.rms.toFixed(4)}</strong></div>
            <div><span>Mel shape</span><strong>{result.mel_spectrogram_shape.join(" × ")}</strong></div>
          </div>
        </section>
      )}
    </main>
